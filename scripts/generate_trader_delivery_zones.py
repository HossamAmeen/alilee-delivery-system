...
def parse_args():
    ...
    p.add_argument("--base-url", default="https://alilee.hossamstore.store", help="Base URL of the API (no trailing slash) or with https://")
    p.add_argument("--token", help="JWT access token to use for Authorization: Bearer <token>")
    ...
    p.add_argument("--min-price", type=float, default=5.0, help="Minimum price for generated entries")
    p.add_argument("--max-price", type=float, default=50.0, help="Maximum price for generated entries")
    p.add_argument("--dry-run", action="store_true", help="Don't actually POST, just print what would be created")
    return p.parse_args()
...
def get_token_by_credentials(session, base_url, email, password):
    ...
    for key in ("token", "access_token", "accessToken"):
        if key in data:
            return data[key]
    print("Token not found in response to login request. Response keys:", list(data.keys()))
    return None
...
def main():
    ...
    if len(creations) < args.count:
        print(f"Could only prepare {len(creations)} unique entries after {tries} attempts.")

    print(f"Prepared {len(creations)} entries. Dry run={args.dry_run}")
    ...
    for trader, zone, price in creations:
        ...
        if r.status_code in (200, 201):
            print(f"Created: trader={trader} zone={zone} price={price}")
