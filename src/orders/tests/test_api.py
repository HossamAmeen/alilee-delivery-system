...
def test_create_order_authenticated(api_client, trader_user, order_data):
    ...
    response = api_client.post(
        list_url, data=json.dumps(order_data), content_type="application/json"
    )
...
@pytest.mark.django_db
def test_list_orders_as_driver(
    api_client, trader_user, driver_user, customer, delivery_zone
):
